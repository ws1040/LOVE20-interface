...
const MyVerifingPanel: React.FC<MyVerifingPanelProps> = ({ currentRound }) => {
    ...
    const { scoreByVerifier, isPending: isPendingScoreByVerifier } = useScoreByVerifier(token?.address as `0x${string}`, currentRound, accountAddress as `0x${string}` || '');

    return (
    <div className="flex flex-col items-center space-y-4 p-6 bg-base-100 mt-4">
        
        <h1 className="text-base text-center">
            验证轮（第 <span className="text-red-500">{Number(currentRound || BigInt(0))}</span> 轮）
        </h1>
        ...
        <div className="flex w-full justify-center space-x-20">
            ...
            {
              isPendingScoreByVerifier ? 
                <Loading /> : 
                formatTokenAmount(scoreByVerifier || BigInt(0))
            }
            </span>
            </div>
            <div className="flex flex-col items-center">
