...
const VotingActionList: React.FC<VotingActionListProps> = ({ currentRound }) => {
  ...
  const { votes, isPending: isPendingVotesNums, error: errorVotesNums } = useVotesNums(token?.address as `0x${string}` || '', currentRound || BigInt(0));
  const { actionSubmits, isPending: isPendingActionSubmits, error: errorActionSubmits } = useActionSubmits(token?.address as `0x${string}` || '', currentRound || BigInt(0));
